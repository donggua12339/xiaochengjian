  ...
  async getConfig(developerId: string, appId: string) {
    ...
    if (!config) {
      ...
      return {
        ...
        soEncrypt: true,
        detectionModules: {
          antiDebug: true, antiFrida: true, antiDump: true,
          rootDetect: true, xposedDetect: true, emulatorDetect: false,
          vpnDetect: true, dualAppDetect: true, fartDetect: false, odexDetect: false,
        },
        killAction: 'kill',
        weakThreshold: 70,
        delayMinMs: 0,
