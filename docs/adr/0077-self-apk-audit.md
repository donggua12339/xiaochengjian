...
| 例外 | 范围 | 约束 | 依据 |
|---|---|---|---|
| 签名回填(例外 A) | 仅 META-INF/ 目录 | 不动 dex/resource/manifest + 自有 keystore + V1+V2+V3 + hash 入白名单 | ADR 0030 + ADR 0070 |
| 梆梆加固自检(例外 B) | 梆梆加固层完整性扫描 | 仅梆梆(锁 A)+ EULA 前置(锁 B)+ 仅完整性报告(锁 C) | ADR 0078 |
| SDK 封装(例外 C) | 注入固定 classes-xcj.dex + Manifest 委托修改 | 六锁架构(对象/内容/入口/签名/权限/数据锁定)+ 律师前置 + 风险熔断 | ADR 0081(proposed) |

例外不改变三重校验的强制性,签名回填、梆梆自检和 SDK 封装仍须先通过三重校验,且审计日志单独记录。

**例外 C(SDK 封装)特别说明**:
- 状态:proposed(律师前置,代码未实现)
- 字节码修改仅限:改 Application superclass + 注入固定 dex + Manifest 委托/Meta-data
