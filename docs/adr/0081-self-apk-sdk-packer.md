...
### v1 适用范围(律师意见书 XCJ-2026-LC-0081 但书 C)

**v1 仅支持未加固自有 APK。**

- 加固 APK(梆梆/360/爱加密等)的"解固→封装"链路 v1 **不打通**,待 v3 另行评估
- 理由:加固厂商 EULA 对"解固后重打包"的态度不明确,加固层算法权属在加固厂商,存在合同违约风险(非刑事,但可能引发商事函告)
- 若开发者 APK 含梆梆加固,应先走 ADR 0078 自检(仅完整性报告),**不进入 packer 封装流程**
- packer 模块在检测到加固 so(libSecShell.so / libjiagu.so 等)时,拒绝封装并返回 `HARDENER_DETECTED_REJECT_PACK`
