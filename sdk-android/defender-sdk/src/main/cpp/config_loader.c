...
x4_config_t g_x4_config = {
    ...
    .soft_exempt_count = 0,
    .hard_exempt_count = 0,
    .strong_switches = { true, true, true, true, true },
};
...
/* 查询接口                                                                */
/* ===================================================================== */
bool                 x4_config_is_enabled(void)        { return g_x4_config.enabled; }
x4_on_violation_t   x4_config_get_on_violation(void)   { return g_x4_config.on_violation; }
...
bool x4_config_is_soft_exempt(const char *pkg) {
    ...
    if (g_x4_config.is_mdm) return true;
...
void x4_config_load(const char *json_path) {
    ...
    LOGI("[X4] config loaded: enabled=%d onViolation=%d dryRun=%d isMDM=%d",
         g_x4_config.enabled, g_x4_config.on_violation,
         g_x4_config.dry_run, g_x4_config.is_mdm);
}
