...
#define STRONG_EVIDENCE_COUNT      ((0xa0u) >> 5)
...
_Static_assert(X4_KILL_THRESHOLD_OVERRIDE <= 80,
               "killThreshold too high, real attacks may slip");
_Static_assert(X4_WARN_THRESHOLD_OVERRIDE >= 30,
               "warnThreshold too low, false warn risk");
_Static_assert(X4_WARN_THRESHOLD_OVERRIDE <= 50,
               "warnThreshold too high, warn becomes useless");
...
_Static_assert(SAFETY_MARGIN >= 15,
               "safety margin too small, risk of single-hit kill");

/* MAX_WEAK_WEIGHT 必须等于 KILL - SAFETY_MARGIN */
_Static_assert(MAX_WEAK_WEIGHT == 50,
               "MAX_WEAK_WEIGHT must equal KILL_THRESHOLD - SAFETY_MARGIN");

/* 衰减系数锁死 0.7 */
_Static_assert(DECAY_FACTOR_NUM == 7 && DECAY_FACTOR_DEN == 10,
               "decay factor must be 0.7");
...
/* 强证据条数锁死 5 */
_Static_assert(STRONG_EVIDENCE_COUNT == 5, "strong evidence count must be 5");
