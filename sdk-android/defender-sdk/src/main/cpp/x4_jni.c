...
static void x4_integrity_init_jni(JNIEnv *env, jobject thiz, jstring apk_path_j)
{
    ...
    x4_integrity_init(p);
    if (apk_path_j)
        (*env)->ReleaseStringUTFChars(env, apk_path_j, p);
}
...
static void x4_init_jni(JNIEnv *env, jobject thiz, jstring config_path_j, jstring self_pkg_j,
                        jstring apk_path_j, jstring expected_hash_j)
{
    (void) thiz;
    const char *config_path =
        config_path_j ? (*env)->GetStringUTFChars(env, config_path_j, NULL) : NULL;
    ...
    const char *expected =
        expected_hash_j ? (*env)->GetStringUTFChars(env, expected_hash_j, NULL) : NULL;
    x4_init(config_path, self_pkg, apk_path, expected);
    if (config_path_j)
        (*env)->ReleaseStringUTFChars(env, config_path_j, config_path);
    if (self_pkg_j)
        (*env)->ReleaseStringUTFChars(env, self_pkg_j, self_pkg);
    if (apk_path_j)
        (*env)->ReleaseStringUTFChars(env, apk_path_j, apk_path);
    if (expected_hash_j)
        (*env)->ReleaseStringUTFChars(env, expected_hash_j, expected);
}
...
static void x8_init_jni(JNIEnv *env, jobject thiz, jstring pkg_j)
{
    ...
    x8_init(p);
    if (pkg_j)
        (*env)->ReleaseStringUTFChars(env, pkg_j, p);
}
...
/* GC 根巡检(ADR 0098 P0-D) */
static jint x4_gc_roots_scan_jni(JNIEnv *env, jobject thiz)
{
    (void) thiz;
    JavaVM *vm = NULL;
    if ((*env)->GetJavaVM(env, &vm) != JNI_OK || vm == NULL)
        return 0;
    return (jint) x4_gc_roots_scan((void *) vm);
}
...
int x4_register_natives(JNIEnv *env)
{
    ...
    JNINativeMethod methods[] = {
        ...
        {"gcRootScan", "()I", (void *) x4_gc_roots_scan_jni},
    };
    jint rc = (*env)->RegisterNatives(env, clazz, methods, 15);
